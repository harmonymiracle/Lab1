...
public class MyGraphUI extends JFrame {
	...
	public final void placeComponents(final JFrame frame) {
	...
	private final String CLASSNAME = "lab_one.MyGraph";
    ...
    public final void actionPerformed(ActionEvent e) {
		...
		if (cmd.equals("CreateMap")) {
			...
			JOptionPane.showMessageDialog(MyGraphUI.this.jf,
		"游走结果如下: " + s, "随机游走", JOptionPane.INFORMATION_MESSAGE);
		} else {
			JOptionPane.showMessageDialog(MyGraphUI.this.jf,
		"未知命令，解析失败", "Failed", JOptionPane.PLAIN_MESSAGE);
		}

	}

	final void CreateGraph(String cmd) {
	...
	/**
	 * @param cmd show graph
	 */
	final void showGraph(final String cmd) {
	    JOptionPane.showMessageDialog(MyGraphUI.this.jf,
	            "请等待, 生成中......", "生成有向图图片",
		...
		if (showOk) {
		    JOptionPane.showMessageDialog(MyGraphUI.this.jf,
		            "已生成，请在当前文件夹查看 ", "生成有向图图片",
		            JOptionPane.INFORMATION_MESSAGE);
		} else {
		    JOptionPane.showMessageDialog(MyGraphUI.this.jf,
		            "生成失败，请检查是否文件过大 ", "生成有向图图片",
		            JOptionPane.INFORMATION_MESSAGE);
	 ...
	 * @param cmd
	 */
	final void queryBridge(String cmd) {
		String word1 = null;
		String word2 = null;
		word1 = JOptionPane.showInputDialog(MyGraphUI.this.jf,
		        "请输入word1： ", "查询两词的桥接词",
		        JOptionPane.INFORMATION_MESSAGE);

		word2 = JOptionPane.showInputDialog(MyGraphUI.this.jf,
		        "请输入word2： ", "查询两词的桥接词",
		        JOptionPane.INFORMATION_MESSAGE);
		...
		if (tempAns.equals(Bridge_Status.ERRORONE)) {
		    ...
		    JOptionPane.showMessageDialog(MyGraphUI.this.jf,
		            "No bridge words from \"" + word1 + "\" to \""
		    + word2 + "\"!", "Fail", JOptionPane.PLAIN_MESSAGE);


		} else if (tempAns.equals(Bridge_Status.ONE_BRIDGE)) {
		    JOptionPane.showMessageDialog(MyGraphUI.this.jf,
		            "The bridge words from \"" + word1 + "\" to \""
		    + word2 + "\"are: " + MyGraphUI.this.graph.Get_Bridge(),
	 ...
	 * @param cmd do
	 */
	final void generateText(String cmd) {
		JDialog jd = new JDialog(MyGraphUI.this.jf);
	...
	final void calMinPath(String cmd) {
		JDialog jd = new JDialog(MyGraphUI.this.jf);
	...
	public final Boolean file2Graph(String methodName, String path) {
	...
	public String generateText(final String methodName, final String oldText) {
	...
	public final String getMinPath(String methodName, String word1, String word2) {
		String minPathStr = "";
	...
	public final String randomWalk(String methodName) {
		String ans = "";
