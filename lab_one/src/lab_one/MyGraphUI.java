...
public class MyGraphUI extends JFrame {
	...
	final String GraphConstructor_CLASSNAME = "lab_one.GraphConstructor";
	final String BridgeWord_CLASSNAME = "lab_one.BridgeWord";
	final String NewText_CLASSNAME = "lab_one.NewText";
	final String ShortestPath_CLASSNAME = "lab_one.ShortestPath";
	final String RandomWalk_CLASSNAME = "lab_one.RandomWalk";
	final String GraphViz_CLASSNAME = "lab_one.GraphViz";
	@Override
	...
	void ShowGraph(String cmd){
		...
		Boolean showOk = graphViz.showDirectedGraph("Result");
		if(showOk){
	...
	void QueryBridge(String cmd){
		...
		BridgeWord tempAns = GetBridgeWord(cmd, word1, word2);
		if(tempAns.isErrorOne()){
			JOptionPane.showMessageDialog(MyGraphUI.this.jf, "No \"" + word1 +"\" in the graph!", "Error", JOptionPane.PLAIN_MESSAGE);
		}
		else if(tempAns.isErrorOneAndTwo()){
		...
		else if(tempAns.isNoneBridge()){
			JOptionPane.showMessageDialog(MyGraphUI.this.jf, "No bridge words from \"" + word1 + "\" to \"" + word2 + "\"!", "Fail", JOptionPane.PLAIN_MESSAGE);
		}
		else if(tempAns.isOneBridge()){
			JOptionPane.showMessageDialog(MyGraphUI.this.jf, "The bridge words from \"" + word1 + "\" to \"" + word2 +"\"are: " + MyGraphUI.this.bridgeWord.getBridge(), "Success", JOptionPane.PLAIN_MESSAGE);
		}
		else if(tempAns.isMultiBridge()){
			String tempArr[] = tempAns.getBridge().split(" ");// MyGraphUI.this.graph.Get_Bridge().split(" ");
			String temp = "";
	...
	public Boolean File2Graph(String methodName, String path){
		...
		try{
			Class<?> model = Class.forName(GraphConstructor_CLASSNAME);
		...
		try{
			createMethod.invoke(MyGraphUI.this.graphConstructor ,sb.toString());
	...
	public BridgeWord GetBridgeWord(String methodName, String word1, String word2){
	    BridgeWord bw = null;
		Method searchBridgeMethod = null;
		try{
			Class<?> model = Class.forName(BridgeWord_CLASSNAME);
		...
		try{
		    ...
		    bw = (BridgeWord)searchBridgeMethod.invoke(MyGraphUI.this.bridgeWord ,word1, word2);
			if(bw != null){
	...
	public String GenerateText(String methodName, String oldText){
		...
		try{
			Class<?> model = Class.forName(NewText_CLASSNAME);
		...
		try{
			...
			ans = (String)GenerateTextMethod.invoke(MyGraphUI.this.newText ,oldText);
	...
	public String GetMinPath(String methodName, String word1, String word2){
		...
		try{
			Class<?> model = Class.forName(ShortestPath_CLASSNAME);
		...
		try{
			minPathStr = (String)MinPathMethod.invoke(MyGraphUI.this.shortestPath ,word1, word2);
			if(minPathStr == null){
			}
			else{
	...
	public String RandomWalk(String methodName){
		...
		try{
			Class<?> model = Class.forName(RandomWalk_CLASSNAME);
		...
		try{
			...
			ans = (String)RandomWalkMethod.invoke(MyGraphUI.this.randomWalk );
			if(ans == null){
